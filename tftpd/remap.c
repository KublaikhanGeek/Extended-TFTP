...
static int parseline(char* line, struct rule* r, int lineno)
{
    ...
    for (p = buffer; *p; p++)
    {
        ...
        case 'P':
            ...
            break;
        default:
            syslog(LOG_ERR, "Remap command \"%s\" on line %d contains invalid char \"%c\"", buffer, lineno, *p);
            return -1; /* Error */
            break;
        }
    }
    ...
    if ((r->rule_flags & (RULE_INVERSE | RULE_REWRITE)) == (RULE_INVERSE | RULE_REWRITE))
    {
...
char* rewrite_string(const char* input, const struct rule* rules, char mode, int af, match_pattern_callback macrosub,
                     const char** errmsg)
{
    ...
    *errmsg = "Remap table failure";
    ...
    for (ruleptr = rules; ruleptr; ruleptr = ruleptr->next)
    {
        ...
        if (!deadman--)
        {
            syslog(LOG_WARNING, "remap: Breaking loop, input = %s, last = %s", input, current);
            free(current);
        ...
        do
        {
            if (regexec(&ruleptr->rx, current, 10, pmatch, 0) == (ruleptr->rule_flags & RULE_INVERSE ? REG_NOMATCH : 0))
            {
                /* Match on this rule */
                was_match = 1;
                ...
                if (ruleptr->rule_flags & RULE_ABORT)
                {
                    ...
                    if (ruleptr->pattern[0])
                    {
                        ...
                        len    = genmatchstring(NULL, ruleptr->pattern, current, pmatch, macrosub);
